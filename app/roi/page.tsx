...
export default function ROIDashboard() {
  ...
  const handleExportPDF = async () => {
    ...
    report.topInsights.forEach((ins, i) => {
      addText(`${i + 1}. ${ins.title} [${ins.category}] — ${ins.relevanceScore}/10`, 10, true, [30, 20, 60]);
      addText(ins.summary, 9, false, [80, 70, 100]);
      addText(`→ Action: ${ins.actionItem}`, 9, false, [124, 58, 237]);
      y += 3;
    });
    addDivider();
    ...
    report.actionItems.forEach((item, i) => {
      addText(`${i + 1}. ${item}`, 9, false, [30, 20, 60]);
    });
    y += 4;
    addDivider();
    ...
    if (report.followUpEmails.length > 0) {
      ...
      report.followUpEmails.forEach(fu => {
        ...
        addText(fu.emailDraft, 9, false, [30, 20, 60]);
        y += 4;
      });
      addDivider();
    }
    ...
    y += 2;
    addText(report.linkedInPost, 9, false, [30, 20, 60]);
    y += 4;
    addDivider();
